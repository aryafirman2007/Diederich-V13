let handler = async(m, { conn, args, usedPrefix }) => {
    if (args.length == 0) return conn.reply(m.chat, `Masukan Code Otp Pendaftaran!!!`, m)
   ...
   if (new Date - global.db.data.users[m.sender].lastcode > 86800000) {
     if (new Date - global.db.data.users[m.sender].lastcode < 86800000) throw `Anda sudah menggunakan kode otp anda, Kode otp anda expired`
    conn.reply(m.chat, `*VERIFY SUSCESS*\n\nAkun Berhasil di buat dan Kamu telah mendapatkan\n+25000 XP\n+25000 Money\n+25000 Nabung Money\n+25 Limit\nSilahkan Ketik *.allmenu* untuk melanjutkan\n\nSelamat menggunakan Bot✨`, m)
